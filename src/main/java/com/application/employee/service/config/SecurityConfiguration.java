...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable)
                ...
                .permitAll()
                .requestMatchers("/user-company/**").hasAnyRole(SADMIN.name(),ADMIN.name(),EMPLOYEE.name(),PROSPECT.name(),RECRUITER.name(),SALES.name(),HR_MANAGER.name())
               .requestMatchers("/employees/**","/orders/**").hasAnyRole(SADMIN.name(),ADMIN.name(),EMPLOYEE.name(), PROSPECT.name(),RECRUITER.name(),SALES.name(),HR_MANAGER.name())
                ...
                .requestMatchers("/timesheets/**","/companies/**").hasAnyRole(SADMIN.name(),ADMIN.name(),HR_MANAGER.name())
                .requestMatchers(GET,"/timesheets/**","/companies/**").hasAnyAuthority(SADMIN.name(),ADMIN.name(),ADMIN_READ.name())
                .requestMatchers(POST,"/timesheets/**","/companies/**").hasAnyAuthority(SADMIN.name(),ADMIN.name(),ADMIN_CREATE.name())
                .requestMatchers(PUT,"/timesheets/**","/companies/**").hasAnyAuthority(SADMIN.name(),ADMIN.name(),ADMIN_UPDATE.name())
                .requestMatchers(DELETE,"/timesheets/**","/companies/**").hasAnyAuthority(SADMIN.name(),ADMIN.name(),ADMIN_DELETE.name())
                .requestMatchers("/email/***", "/contacts/**","/bulkmails/**").hasAnyRole(SADMIN.name(),ADMIN.name(), RECRUITER.name(),HR_MANAGER.name())
                .requestMatchers("/address-verification/**").hasAnyRole(SADMIN.name(),ADMIN.name(),HR_MANAGER.name(),EMPLOYEE.name())
                .requestMatchers("/timesheets/reminders/**").hasAnyRole(SADMIN.name(),ADMIN.name(),HR_MANAGER.name())
