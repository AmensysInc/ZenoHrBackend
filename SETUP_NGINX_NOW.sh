#!/bin/bash
# Quick setup script for zenopayhr.com nginx configuration

echo "Creating nginx configuration for zenopayhr.com..."

# Create the configuration file
sudo tee /etc/nginx/sites-available/zenopayhr.com > /dev/null <<'EOF'
...
server {
    ...
    # SSL Configuration (uncomment after getting certificate)
    ...
    location /api {
        ...
        # CORS headers (backend should handle this, but adding here as backup)
...
sudo ln -sf /etc/nginx/sites-available/zenopayhr.com /etc/nginx/sites-enabled/
...
if [ $? -eq 0 ]; then
    ...
    echo "Run: sudo certbot --nginx -d zenopayhr.com -d www.zenopayhr.com"
else
